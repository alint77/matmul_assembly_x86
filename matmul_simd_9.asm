...
section .data
    ...
    align 64
    b_matrix_rmaj times 1152*1152 dd -3.1;
...
.loop_b_cols:
    ...
    .loop_dotprod: ; 6x2(x8) kernel:  8 mem reads for 12(x8) c elements => 0.66 mem reads per element 
        ...
        vmovaps ymm0,  [b_matrix_rmaj+r12 + r9*4     ]   ;  ymm0 = b[k][j:j+8)   k*b_cols*4 + j
        vmovaps ymm1,  [b_matrix_rmaj+r12 + r9*4 + 32]   ;  ymm1 = b[k][j+8,j+16)
        ...
        add rax,rbx
        vbroadcastss ymm2,  [rax] ;  ymm2 = a[i+1][k] = a[(i+1)*a_cols*4 + k*4]
        vfmadd231ps ymm13,ymm2,ymm0 ; c[i+1][j]
        vfmadd231ps ymm12,ymm2,ymm1 ; c[i+1][j+1]

        add rax,rbx
        vbroadcastss ymm2,  [rax] ;  ymm2 = a[i+2][k] = a[(i+2)*a_cols*4 + k*4]
        vfmadd231ps ymm11,ymm2,ymm0 ; c[i+2][j]
        vfmadd231ps ymm10,ymm2,ymm1 ; c[i+2][j+1]

        add rax,rbx
        vbroadcastss ymm2,  [rax] ;  ymm2 = a[i+3][k] = a[(i+3)*a_cols*4 + k*4]
        vfmadd231ps ymm9,ymm2,ymm0 ; c[i+3][j]
        vfmadd231ps ymm8,ymm2,ymm1 ; c[i+3][j+1]

        add rax,rbx
        vbroadcastss ymm2,  [rax] ;  ymm2 = a[i+4][k] = a[(i+4)*a_cols*4 + k*4]
        vfmadd231ps ymm7,ymm2,ymm0 ; c[i+4][j]
        vfmadd231ps ymm6,ymm2,ymm1 ; c[i+4][j+1]

        add rax,rbx
        vbroadcastss ymm2,  [rax] ;  ymm2 = a[i+5][k] = a[(i+5)*a_cols*4 + k*4]
        vfmadd231ps ymm5,ymm2,ymm0 ; c[i+5][j]
        vfmadd231ps ymm4,ymm2,ymm1 ; c[i+5][j+1]
